...
def frc_asset_register(request):
    ...
    df=df[['Financial Year', 'Purchase date', 'Sl ', 'Bill no','Economic Code',
       ...
       'Modified Number', 'Price', 'Sold (unit)','Sold Unit(Modified)','FY of Items sold',
    ...
    if(len_csv_t > 1):
        ...
        df["Asset Code"]="FRC" + "-" +  df.iloc[:, 18].astype(str).iloc[0]+"-"+df.iloc[:, 17].astype(str).iloc[0] + "-" + df.iloc[:, 5].astype(str).str[:1] + "-" +df.iloc[:, 1].astype(str).str.extract(r'(\d{2})(\d{2})').iloc[:, 1] + "-" +df.iloc[:, 2].astype(str).apply(lambda x: x.zfill(4))
